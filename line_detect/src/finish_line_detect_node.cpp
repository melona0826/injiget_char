/***********************************************************************
 * finish_line_detect_node.cpp
 *
 * Author : Kwon Jin (School of Computer, KAIST)
 * Date (Last Modify):2023.06.15
 *
 * Finish line detecting Node.
 * Detect finish line with hough line detection.
 *
 **********************************************************************/
#include <ros/ros.h>
#include <iostream>
...
int blue_hue_low = (int)(180 / 2);
int blue_hue_high = (int)(230 / 2);
int blue_sat_low = 55 * (int)(255 / 100);
int blue_sat_high = 100 * (int)(255 / 100);
int blue_val_low = 55 * (int)(255 / 100);
int blue_val_high = 100 * (int)(255 / 100);
...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "finish_line_detect");
  ...
  */
   ...
   *   (NN : "/finish_line_detect/line_pos")
   *
   * pub      :
   *   detected image message publisher (image_transport Publisher)
   *   (NN : "/finish_line_detect/detect_img")
   *
   * pub2      :
  ...
  */
  ...
  [&](const sensor_msgs::ImageConstPtr& msg){
    if(toggle)
    {
      ...
      inRange(img_hsv, Scalar(blue_hue_low, blue_sat_low, blue_val_low) , Scalar(blue_hue_high, blue_sat_high, blue_val_high), line_mask);
      ...
      for(size_t i = 0; i < lines.size(); i++)
      {
        ...
        // select line only under the slope treshold
      ...
      if(pts.size() > 0)
      {
        ...
        */
        ...
        fitLine_msg.x = b.x;
        fitLine_msg.y = b.y;
        fitLine_msg.theta = m * 180 / CV_PI;
        // cout << "m : " << m * 180 / CV_PI  << endl;
        // //double te = (static_cast<double>(pt1_y) - static_cast<double>(pt2_y)) / (static_cast<double>(pt1_x) - static_cast<double>(pt2_x));
        // //cout << te * 180 / CV_PI << endl;
      }
